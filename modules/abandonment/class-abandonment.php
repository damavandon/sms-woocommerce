...
class Abandonment
{
    ...
    public function __construct()
    {
        ...
        if ($option['active'] == true && !isset($_COOKIE['pwc_abandonment_ca_skip_track_data'])) {
            ...
            add_action('wp_ajax_abandonment_data', array($this, 'save_cart_abandonment_data'));
            add_action('wp_ajax_nopriv_abandonment_data', array($this, 'save_cart_abandonment_data'));
            ...
            add_action('woocommerce_order_status_changed', array($this, 'pwc_abandonment_ca_update_order_status'), 999, 3);
            ...
            add_filter('cron_schedules', array($this, 'add_cron'));

            if (!wp_next_scheduled('payamito_wc_abandoned')) {
                wp_schedule_event(time(), 'every_one_minutes', 'payamito_wc_abandoned');
            }
            add_action('payamito_wc_abandoned', array($this, 'abandonment'));
    ...
    public function cart_abandonment_tracking_script()
    {
        ...
        global $post;
        wp_enqueue_script(
            'payamito-wc-cart-abandonment',
            PAYAMITO_WC_Module_URL . '/abandonment/assets/js/cart-abandonment.js',
            array('jquery'),
            PAYAMITO_WC_ABANDONED,
            true
        );
    ...
    public function abandonment_completed($session_id, $template_id, $arg)
    {
        ...
        $queue = array_column(unserialize($results['queue']), 'id');
    ...
    public function abandonment_history_update($sended, $arg, $queue)
    {
        ...
        if (!in_array($arg['pattern_id'], $ids)) {
            ...
            $wpdb->update(
                DB::table_name("history"),
                ['queue' => serialize($sended_queue)],
                ['id' => $sended['id']]
            );
        }
    }
                    ...
                    if ($template['method'] === 'pattern') {
                        foreach ($template['patterns'] as $pattern) {
                            ...
                            if ($this->is_time_send($current_session_id, $template['id'], $abandonment['time'], $pattern['frequency'])) {
                                ...
                                global $wpdb;
                                $queue = [];
                                array_push($queue, ['id' => $pattern['id'], 'send_time' => date_i18n("Y-m-d H:i:s", current_time('timestamp'), true)]);
                                ...
                                if (is_array($sended)) {
                                    $send =$this->prepare_pattern_send($phone, $abandonment, $pattern,  $coupon_data);
                                    if ($send === true) {
                                        $this->abandonment_history_update($sended, ['pattern_id' => $pattern['id'], 'phone' => $phone], $queue);
                                    }
                                }
                            }
                        }
    ...
    public function skip_future_sms_when_order_is_completed($session_id, $skip_complete = false)
    {
        ...
        $wpdb->update($email_history_table, array('email_sent' => -1), array('session_id' => $session_id, 'email_sent' => 0,));
    }
    ...
    public function get_checkout_details($pwc_abandonment_session_id)
    {
        ...
        $cart_abandonment_table = DB::table_name("cart_abandonment");
        $result                 = $wpdb->get_row(
            $wpdb->prepare('SELECT * FROM `' . $cart_abandonment_table . '` WHERE session_id = %s', $pwc_abandonment_session_id) // phpcs:ignore
        );
        return $result;
    }
    ...
    public function get_checkout_details_by_phone($phone)
    {
        ...
        $cart_abandonment_table = DB::table_name("cart_abandonment");
        $result                 = $wpdb->get_row(
            $wpdb->prepare('SELECT * FROM `' . $cart_abandonment_table . '` WHERE phone = %s AND `order_status` IN ( %s, %s )', $phone, PWCAB_CART_ABANDONED_ORDER, PWCAB_CART_NORMAL_ORDER) // phpcs:ignore
        );
        return $result;
    }
    ...
    public function get_captured_data_by_phone($value)
    {
        ...
        $result                 = $wpdb->get_row(
            $wpdb->prepare(
                'SELECT * FROM `' . $cart_abandonment_table . '` WHERE phone = %s AND `order_status` IN (%s, %s) ORDER BY `time` DESC LIMIT 1',
                $value,
                PWCAB_CART_ABANDONED_ORDER,
                PWCAB_CART_LOST_ORDER
            )
        );
        return $result;
    }
    ...
    public function get_tracked_data_without_status($value)
    {
        ...
        $result                 = $wpdb->get_row(
            $wpdb->prepare(
                'SELECT * FROM `' . $cart_abandonment_table . '` WHERE phone = %s LIMIT 1',
                $value
            ) // phpcs:ignore
        );
        return $result;
